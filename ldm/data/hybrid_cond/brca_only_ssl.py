...
class TCGADataset(Dataset):
    ...
    def __init__(self, config=None):
        ...
        # load SSL features
        features = h5py.File(self.root / "ssl_features/features.h5", "r")
        self.feat_20x = features['feat_20x']
        self.feat_5x = features['feat_5x']
    ...
    def __getitem__(self, idx):
        ...
        idx_5x = self.indices_5x[idx]
        feat_5x = self.feat_5x[idx_5x].astype(np.float32)
        ...
        if np.random.rand() < self.p_uncond:
            feat_5x = np.zeros_like(feat_5x)


        if self.return_5x_img:
            name_5x = self.image_root / ('/'.join(img_path.split("/")[:-1]) +".jpg")
            img_5x = np.array(Image.open(name_5x))
            img_5x = (img_5x / 127.5 - 1.0).astype(np.float32)
            return {
                "image": image,
                "feat_20x": feat_20x,
                "feat_5x": feat_5x,
